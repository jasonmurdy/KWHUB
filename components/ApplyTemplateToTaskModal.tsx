...
const ApplyTemplateToTaskModal: React.FC = () => {
  const { templateToApplyToTask, closeApplyTemplateToTaskModal, templates, applyTemplateToTask } = useAppContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!templateToApplyToTask || !selectedTemplateId) return;
    setIsLoading(true);
    try {
      await applyTemplateToTask(selectedTemplateId, templateToApplyToTask.id, { importTasks, importLinks });
      closeApplyTemplateToTaskModal();
    } catch (error) {
      console.error("Failed to apply template to task:", error);
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="p-6 border-b border-outline/30 flex justify-between items-center">
            <div>
                <h2 className="text-xl font-bold text-on-surface">Apply Template to Task</h2>
                <p className="text-sm text-on-surface-variant">&quot;{templateToApplyToTask.title}&quot;</p>
            </div>
            <button type="button" onClick={closeApplyTemplateToTaskModal} className="text-on-surface-variant hover:text-on-surface p-2 rounded-full hover:bg-on-surface/5">
          ...
          <div className="p-6 space-y-6">
            <div>
              <label htmlFor="template" className="block text-sm font-medium text-on-surface-variant mb-1">Select Template</label>
              <select 
                id="template" 
            ...
            <div>
                ...
                <div className="space-y-3">
                    <label htmlFor="importTasks" className="flex items-center gap-3 p-3 bg-surface-variant/40 rounded-lg border border-outline/30 cursor-pointer">
                        ...
                        <div>
                            <p className="font-medium text-on-surface text-sm">Import Checklist as Subtasks</p>
                            <p className="text-xs text-on-surface-variant">{selectedTemplate.tasks.length} subtasks will be created.</p>
                        </div>
                    </label>
                     <label htmlFor="importLinks" className="flex items-center gap-3 p-3 bg-surface-variant/40 rounded-lg border border-outline/30 cursor-pointer">
                        ...
                        <div>
                            <p className="font-medium text-on-surface text-sm">Import Links as Attachments</p>
                            <p className="text-xs text-on-surface-variant">{selectedTemplate.links.length} links will be attached.</p>
                        </div>
                    </label>
                </div>
            </div>
          ...
          <div className="p-4 border-t border-outline/30 bg-surface/50 rounded-b-3xl flex justify-end items-center">
            <button
              type="submit"
              disabled={isLoading || !selectedTemplateId || (!importLinks && !importTasks)}
...
export default ApplyTemplateToTaskModal;
